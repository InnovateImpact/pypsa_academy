...
def convert_excel_to_csv(excel_file_path, csv_folder_path):
    ...
    # Initialize a new PyPSA network and define a set for components
    # n = pypsa.Network()
    # components = {
    #     f"{n.components[key]['list_name']}-{attr}" if attr else n.components[key]['list_name']
    #     for key in n.component_attrs
    #     for attr in list(n.component_attrs[key][
    #         (n.component_attrs[key]['status'].str.contains("Input")) & 
    #         (n.component_attrs[key]['varying'] == True)
    #     ].index) or [None]
    #     }.union({n.components[key]['list_name'] for key in n.component_attrs})
